...
>    - Install Docker by running this command `sudo snap install docker`<br>
...
>        PUSHER_APP_SECRET=
>        
>        EMAILS_FROM_NAME='Campusbibliothek Brugg-Windisch'
...
>      9.    ```txt
>            https://user:<<TOKEN>>@gitlab.fhnw.ch/thekre/ThekRe.git
>            ```
...
>    To run Docker commands without sudo, add your user to the docker group. You can do this by running the following command: <br>
    ...
    ```sudo usermod -a -G docker $USER``` <br>
     ...
     ```sudo chown -R root:docker /var/run/docker``` <br>  
>    After that you need to logout and login again to apply the changes. <br>
>    You can test it by running `docker run hello-world` in the terminal. If you get a "Hello from Docker!" message, everything is set up correctly. (src: https://askubuntu.com/questions/477551/how-can-i-use-docker-without-sudo) <br> 
...
>     myorigin = /etc/mailname
>     mydestination = $myhostname, server1121.cs.technik.fhnw.ch, localhost.cs.technik.fhnw.ch, , localhost
>     relayhost =
